...
class PzkConfigAdminController extends PzkBackendController {
    ...
    public function writePostAction() {
        ...
        if(pzk_session()->getStoreType() == 'app') {
        	...
        	file_put_contents(BASE_DIR . "/app/".pzk_app()->getPathByName()."/configuration.".pzk_request()->getSoftwareId().".php", '<?php pzk_store_instance("'.pzk_request()->getAppPath() . '/' . pzk_request()->getSoftwareId() .'")->set(\'config\', '.var_export($config, true) . ');');
        }
