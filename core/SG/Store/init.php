...
function pzk_controllercache($key = NULL, $value = NULL, $timeout = null) {
	static $store;
	if (! $store)
		$store = new PzkSGStoreLazy(new PzkSGStoreFormatJson ( new PzkSGStoreDomain ( new PzkSGStoreDriverFile ( 'cache/controller' ) ) ));
	if ($key === NULL) {
		return $store;
	} else {
		if ($value === NULL) {
...
function pzk_layoutcache($key = NULL, $value = NULL, $timeout = null) {
	static $store;
	if (! $store) {
		// if(CACHE_MODE) {
			$store = new PzkSGStoreLazy(new PzkSGStoreFormatJson ( new PzkSGStoreDomain ( new PzkSGStoreDriverFile ( 'cache/layout' ) ) ));
		// } else {
			// $store = new PzkSGStoreDriverPhp();
		// }
	}
