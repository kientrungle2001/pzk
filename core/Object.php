	...
	*/
	...
	public function css() {
		if ($this->css != false) {
			if($this->scriptTo) {
				$elem = pzk_element($this->scriptTo);
				if($cssPath = pzk_layoutcache($this->css . '.css.path')) {
					...
					if(is_file('Default/skin/'.$app->getPathByName() . '/themes/' . $request->get('defaultTheme') .'/css/'.$this->css.'.css')) {
						$elem->append(pzk_obj('Html.Css', array( 
							...
							pzk_layoutcache($this->css . '.css.path', $cssPath);
	...
	public function javascript() {
		...
		if ($this->js != false) {
			if($this->scriptTo) {
				$elem = pzk_element($this->scriptTo);
				if($jsPath = pzk_layoutcache($this->js.'.js.path')) {
					...
					if(is_file('Default/skin/'.pzk_app()->getPathByName().'/js/'.$this->js.'.js')) {
						...
						pzk_layoutcache($this->js.'.js.path', $jsPath);
	...
	public function getLayoutRealPath() {
		if(pzk_layoutcache()->get($this->layout)) {
			$path = pzk_layoutcache()->get($this->layout);
			return $path;
		}
		$layout = null;
		$request = pzk_request();
		...
		if(CACHE_MODE) {
			pzk_layoutcache()->set($this->layout, $layout);
		}
	...
	public function getCssRealPath() {
		if(CACHE_MODE && pzk_layoutcache()->get($this->css)) {
			$path = pzk_layoutcache()->get($this->css);
			return $path;
		}
		$css = null;
		$request = pzk_request();
		...
		if(CACHE_MODE) {
			pzk_layoutcache()->set($this->css, $css);
		}
		return $css;
	}
