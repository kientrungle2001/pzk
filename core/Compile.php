...
function copyGlob($pattern, $dir = 'compile/objects') {
	...
	foreach($files as $file) {
		...
		if(!is_file($fileTarget) || (filemtime(utf8_decode($file)) > filemtime(utf8_decode($fileTarget))))
...
function readNode($node, &$index = 0) {
	...
	$content .= "if(PHAR_MODE)
			require_once 'phar://myapp.phar/compile/objects/$className.php';
		else
...
function compileXmlFile($file, $regenerate = true) {
	...
	echo $file . '<br />';
	echo $fileName . '<br />';
	if(!is_file($fileName) || (filemtime($file) > filemtime($fileName))) {
