PzkCoreDbGrid = PzkObj.pzkExt({
	...
	checkDisplayPadding: function() {
		...
		if(!!window.sessionStorage.getItem(col)) {
			...
			$('#admin_table_list td input, #admin_table_list th input').css('margin-bottom', '0px');
		} else {
	...
	toggleLabel: function () {
		...
		this.checkDisplayLabel();
	},
	checkDisplayLabel: function() {
		var fieldIndex = 'label-display';
		var col = 'grid-column-' + this.id + '-' + this.module + '-' + fieldIndex;
		if(!!window.sessionStorage.getItem(col)) {
			$('#grid-nav label, #grid-nav label ~ br').css('display', 'none');
		} else {
			$('#grid-nav label, #grid-nav label ~ br').css('display', 'inline-block');
		}
	},
