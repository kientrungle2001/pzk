...
class PzkDemoController extends PzkController {
	...
	public function selectorAction() {
		$arr = pzk_parse_selector('arr[name=1][id=2]');
		var_dump($arr);
	}
	...
	public function rankAction(){
		set_time_limit(0);
		$indexing = pzk_model('Indexing');
		$indexing->rankAll();
		return true;
		$book = _db()->selectAll()->fromUser_book()->result('table');
		$rankTests = array();
		foreach($book as $item) {
			$rankTest = null;
			if(isset($rankTests[$item['userId']][$item['testId']])) {
				$rankTest = $rankTests[$item['userId']][$item['testId']];
			} else {
				$rankTests[$item['userId']][$item['testId']] = $rankTest = _db()->getTableEntity('rank_test')->loadWhere(array('and', array('userId' => $item['userId']), array('testId' => $item['testId'])));
			}
			
			if($rankTest && $rankTest->get('id')) {
				if($rankTest->getMark() < $item['mark']) {
					$rankTest->update(array(
						'startTime' => $item['startTime'],
						'duration' => $item['duringTime'],
						'mark' => $item['mark']
					));
				} else if($rankTest->getMark() == $item['mark']) {
					if ($rankTest->getDuration() > $item['duringTime']) {
						$rankTest->update(array(
							'startTime' => $item['startTime'],
							'duration' => $item['duringTime'],
							'mark' => $item['mark']
