<?php 
class PzkArtController extends PzkFrontendController 
{
	...
	public function saveBookAction(){
		...
		if($key_test==pzk_session('keyBook')){
			if($this->checkSave($key_test)){
				...
				$row= array('userId'=>$userId,'categoryId'=>$category_id,'time'=>$time,'quantity_question'=>$quantity_question,'date'=>$date,'mark_status'=>0,'keybook'=>$key_test,'time_real'=>'');
				...
				$answerDetailText= _db()->getTableEntity('user_answers_text');
				...
				foreach ($question_id as $question) {
					...
					$rowAnswer=array('user_book_id'=>$answerTestId,'questionId'=>$questionId,'question_type'=>$questionType,'content'=>$answer[$question]);
