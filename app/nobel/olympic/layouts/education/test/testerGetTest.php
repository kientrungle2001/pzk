...
<?php
	...
	$curentTest = $data->getCurentTest($testId);
	$tests = $data->getTestOtherByClass($curentTest['classes'], $curentTest['id']);
	$keybook = $data->getKeybook();
?>
<div class='bg-white pdb50'>
<div class="container mgb50 left-content">
	
	<div style="color: #0074ed;" class='breadcum'>
					...
					<div class='row'> 
						<div class="col-md-4 pd-0">
							<select onchange="clearTimer(this)" id="lessons" name="lesson" class="form-control select_type mgt5 title-blue" style="text-align:left;" >
								<option selected value = '<?= $curentTest['id']; ?>'><?= $curentTest['name']; ?></option>
								<?php if($tests) : ?>
									<?php foreach($tests as $key =>$value):?>
										<option lessontype="<?php echo $value['questiontype']; ?>" class="pd-left-10" value="tktest"><?=$value['name']?> </option>
									<?php endforeach;?>
								<?php endif;?>
							</select>
						</div>
						<div class='col-xs-12 col-md-4 title-red'>
							...
							<div id="countdown" class ='num_time '> <?=$curentTest['time'];?>:00 </div> 
							<input type='hidden' value="<?= $curentTest['time'];?>" name='lessonTime' id="lessonTime" />
							</div>
						</div>
	...
	<script>
		...
		function getLesson(that) {
			...
			var currentclass = '<?=trim($curentTest['classes'],',');?>';
			if(testId) {
				$.ajax({
					type: "POST",
