...
if($dataWord){
    ...
    $arrWord = array_chunk($arrWord, 8);
    $totalCustomData = count($arrWord)-1;
    $allWord = json_encode($arrWord, JSON_UNESCAPED_UNICODE );
    ...
    <script id="editable">
        ...
        RainWord = {
            ...
            buildStart: function() {
                ...
                background.graphics.beginFill("#337ab7").drawRoundRect(0, 0, 130, 40, 10);
            ...
            skys: function() {
                ...
                this.sky.graphics.drawRect(0, 0, this.canvas.width, this.canvas.height);
            ...
            rainWord: function(dataword){
                ...
                for(i = 0; i < dataword.length; i++) {
                    ...
                    var txt = new TextLink(word, "bold 18px Arial", color, "#000000");
                    txt.cursor = 'pointer';
                    var j = i +1;
                    if(j == 9) {
                        j = 1;
                    ...
                    txt.x = (j * 90)-75  ;
                    txt.y = 35 + Math.floor((Math.random() * 10) + 1)*15;
                    ...
                    if(checkTrue >= 0) {
                        createjs.Tween.get(txt).to({ alpha:1, y: this.stage.canvas.height+10 },15000 + Math.random()*2000*i).call(function(){that.handleComplete();});
                    }else {
                        createjs.Tween.get(txt).to({ alpha:1, y: this.stage.canvas.height+10 },15000 + Math.random()*2000*i).call(function(){that.checkEndWord();});
                    }
                }
            },
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if(data) {
                            ...
                            gamescore.y = stage.canvas.height / 2-12;
