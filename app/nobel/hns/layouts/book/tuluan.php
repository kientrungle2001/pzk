...
<?php
	...
	$contentStdent = preg_replace_callback($pattern, function($matches) {
		...
		return '
		<input disabled size="' . @$matches[4] . '" value="'.$answer['i'][$matches[2]].'" />';
	}, $items['name']);
	...
	$contentStdent = preg_replace_callback($pattern, function($matches) {
		...
		return '
		<input disabled class="input_dt" size="' . @$matches[4] . '" value="'.@$answer['i'][@$matches[2]].'" />';
	}, $contentStdent);
...
<div class="panel-body" style="min-height: 350px">
	<div class="col-xs-12">
		...
		<i><?=$items['request']?> : </i>
		<p><h3>Bài làm của học sinh</h3></p>
		<p><?= $contentStdent;?></p>
		<p><h3>Giáo viên sửa</h3></p>
