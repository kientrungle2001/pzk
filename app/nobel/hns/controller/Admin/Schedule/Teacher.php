...
class PzkAdminScheduleTeacherController extends PzkBackendController {
	...
	public function gotoStudentAction($bookId) {
		...
		$subject = _db()->getTableEntity('categories')->load($book->get('categoryId'));
		$topic 	= _db()->getTableEntity('categories')->load($book->get('topic'));
		$homework = $book->get('testId');
		
		$this->redirect(BASE_REQUEST . '/practice/class-'.$class.'/subject-' . $subject->get('alias') . '-' . $subject->get('id') . '/topic-'.$topic->get('alias') . '-' .$topic->get('id') . '/homework-'.$homework);
	}
	...
	public function addHomeworkToClassroomsAction($classroomId, $classroomHomeworkId, $homeworkId) {
		$classroomIds = pzk_request('classroomIds');
