...
class PzkAdminQuestion2Controller extends PzkGridAdminController {
    ...
    public $filterFields = array(
		...
		array(
            ...
            'option' 		=> array(
                ...
                QUESTION_TYPE_TULUAN_DIENTU => "Tự luận điền từ",
	...
	*/
	...
	function detailAction($id) {
		...
		// dien tu vao cho trong - nhieu dap an
		}elseif($type == QUESTION_TYPE_FILL){
		...
		// tu luan lam bai
			...
			// pzk_system()->halt('admin/'.pzk_or($this->customModule, $this->module).'/question_answers_tl/answersTuluan');
			
			$module = $this->parse('admin/grid/question_answers_tl/answersTuluan');
		...
		// tu luan dien tu
		} elseif($type == QUESTION_TYPE_TULUAN_DIENTU) {
			$module = $this->parse('admin/'.pzk_or($this->customModule, $this->module).'/question_answers_tl/answersTuluanDientu');
			$module->setItemId(pzk_request()->getSegment(3));
			$this->initPage()->append($module);
			$this->display();
		}
