<div class="container user top14">
	<div class="row">
		<div class="col-md-1">&nbsp;</div>			
		<div class="col-xs-11 col-md-11 ">
			<div class="bannerps">
				<h1>FULL LOOK</h1>	
				<div class="btncon">
					<a href="/home/practice"><button type="button" class="btn  btn-custom ">Vào phần mềm</button></a>
					<a href="/home/practice"><button type="button" class="btn   btn-custom">Dùng thử</button></a>
					<a href="/home/about"><button type="button" class="btn   btn-custom">Chi tiết và mua sản phẩm</button></a>
				</div>
			</div>
		</div>
	</div>
</div>
<div class="container top120 fontsize19">
	<h4 class="text-center"><strong>Thông tin cá nhân</strong></h4>
	<div class="row top20">
		<div class="co-md-4 col-xs-4 avatar">
			<img src="<?=BASE_SKIN_URL?>/Default/skin/nobel/test/Themes/Default/media/11.jpg" alt="trang cá nhân" class="img-circle center-block" style="width:120px; height:120px;">
			<a href="#"><button type="button" class="btn btn-default sharp btn-custom center-block top10">Đổi avatar</button></a>
		</div>
		<div class="co-md-8 col-xs-8">
			<ul class="list-unstyled">
				<li><strong>Họ và tên:</strong> Lê Trung Kiên</li>
				<li><strong>Nick name:</strong> Kienle</li>
				<li><strong>Ngày sinh:</strong> 02/02/2002</li>
				<li><strong>Giới tính:</strong> Nam</li>
				<li><strong>Địa chỉ:</strong> 123 Đường Ba Đình, Quận Ba Đình, TP Hà Nội</li>
				<li><strong>Trường:</strong> Tiểu học Trương Định</li>
				<li><strong>Thời hạn sản phẩm 1 năm</strong></li>
			</ul>
			<div class="btncon pull-right top20">
				<a href="#" ><button type="button" class="btn btn-primary sharp">Sửa thông tin</button></a>
				<a href="#"><button type="button" class="btn btn-default sharp btn-custom">Lưu</button></a>
				<a href="#"><button type="button" class="btn btn-default sharp btn-custom">Hủy</button></a>
			</div>
		</div>
	</div>
</div>
<div class="container top30 bottom20 fontsize19">
	<h4 class="text-center"><strong>Lịch sử học tập</strong></h4>
	<div class="row">
		<div class="col-xs-1 col-md-1"></div>
		<div class="col-xs-10 col-md-10">
			      
  <table class="table table-striped">
    <thead>
      <tr>
        <th>Đề đã làm</th>
        <th>Điểm</th>
        <th>Xếp hạng</th>
		<th>Thời gian làm</th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td>John</td>
        <td>Doe</td>
        <td>john@example.com</td>
		<td>john@example.com</td>
      </tr>
      <tr>
        <td>Mary</td>
        <td>Moe</td>
        <td>mary@example.com</td>
		<td>john@example.com</td>
      </tr>
      <tr>
        <td>July</td>
        <td>Dooley</td>
        <td>july@example.com</td>
		<td>john@example.com</td>
      </tr>
	  <tr>
        <td>John</td>
        <td>Doe</td>
        <td>john@example.com</td>
		<td>john@example.com</td>
      </tr>
	  <tr>
        <td>John</td>
        <td>Doe</td>
        <td>john@example.com</td>
		<td>john@example.com</td>
      </tr>
	  <tr>
        <td>John</td>
        <td>Doe</td>
        <td>john@example.com</td>
		<td>john@example.com</td>
      </tr>
	  <tr>
        <td>John</td>
        <td>Doe</td>
        <td>john@example.com</td>
		<td>john@example.com</td>
      </tr>
	  <tr>
        <td>John</td>
        <td>Doe</td>
        <td>john@example.com</td>
		<td>john@example.com</td>
      </tr>
	  <tr>
        <td>John</td>
        <td>Doe</td>
        <td>john@example.com</td>
		<td>john@example.com</td>
      </tr>
	  <tr>
        <td>John</td>
        <td>Doe</td>
        <td>john@example.com</td>
		<td>john@example.com</td>
      </tr>
    </tbody>
  </table>
		</div>
		<div class="col-xs-1 col-md-1"></div>
	</div>
</div>