				...
				<?php if(!empty($category['child'])):?>
					...
					<button class="btn fix_hover btn-default col-md-12 col-sm-12 col-xs-12 sharp" type="button"><span id="chonde" class="fontsize19"> Chọn bài</span><img class="img-responsive imgwh hidden-xs hidden-sm pull-right" src="<?=BASE_SKIN_URL?>/Default/skin/nobel/Themes/Story/media/icon1.png" /></span>
						...
						<?php if($category_id == 88) {
							...
							<li><a onclick="subject = {value[id]};document.getElementById('chonde').innerHTML = '{value[name]}';" data-de="{value[name]}" class="getdata" href="/practice/doQuestion/{value[id]}?class=5&de={value[name]}" data-type="group"><?php if(pzk_user_special()): ?>#{value[id]}<?php endif;?> - {value[name]} <?php if($check == 0){ echo "Bài dùng thử";} ?></a></li>
						<?php endforeach;
						} else { ?>
							<?php 
								...
								if($level== '1'){
									$practices = $data->getPractices($class,$subject, $check);
										for($i = 1; $i <= $practices; $i++){  ?>
											<li ><a style="padding-left: 40px;" onclick="document.getElementById('chonde').innerHTML = '<?php echo "Bài ".$i; ?>'; de={i}; " data-de="<?php echo $i; ?>" class="getdata" href="/practice/class-5/subject-{subjectEntity.get('alias')}-{subject}/examination-{i}"><?php echo "Bài ".$i;?><?php if($check == 0){ echo " - Bài dùng thử"; }?></a></li>
										<?php } //end for
								}elseif($level== '2'){
									$topics= $data->getTopics($subject, $check);
										...
										foreach ($topics as $topic) {
											...
											for($i = 1; $i <= $practices; $i++){  ?>
												<li ><a style="padding-left: 40px;" onclick="document.getElementById('chonde').innerHTML = '<?php echo "Bài ".$i; ?>'; de={i}; " data-de="<?php echo $i; ?>" class="getdata" href="/practice/class-5/subject-{subjectEntity.get('alias')}-{subject}/topic-{topic[alias]}-{topic[id]}/examination-{i}"><?php echo "Bài ".$i;?><?php if($check == 0){ echo " - Bài dùng thử"; }?></a></li>
											<?php } //end for
										} //end foreach
									...
									foreach ($sections as $section) {
										...
										if(count($topicChilds) == 0) {
											...
											for($i = 1; $i <= $practices; $i++){  ?>
												<li ><a style="padding-left: 40px;" onclick="document.getElementById('chonde').innerHTML = '<?php echo "Bài ".$i; ?>'; de={i}; " data-de="<?php echo $i; ?>" class="getdata" href="/practice/class-5/subject-{subjectEntity.get('alias')}-{subject}/topic-{section[alias]}-{section[id]}/examination-{i}"><?php echo "Bài ".$i;?><?php if($check == 0){ echo " - Bài dùng thử"; }?></a></li>
											<?php 
											}
										} else {										
									...
									foreach ($sections1 as $section1) {
										...
										if(count($sections2) == 0) {
											...
											for($i = 1; $i <= $practices; $i++){  ?>
												<li ><a style="padding-left: 50px;" onclick="document.getElementById('chonde').innerHTML = '<?php echo "Bài ".$i; ?>'; de={i}; " data-de="<?php echo $i; ?>" class="getdata" href="/practice/class-5/subject-{subjectEntity.get('alias')}-{subject}/topic-{section1[alias]}-{section1[id]}/examination-{i}"><?php echo "Bài ".$i;?><?php if($check == 0){ echo " - Bài dùng thử"; }?></a></li>
											<?php 
											}
										} else {
											foreach ($sections2 as $section2) {
												...
												if(count($topicChilds) == 0) {
													...
													for($i = 1; $i <= $practices; $i++){  ?>
														<li ><a style="padding-left: 50px;" onclick="document.getElementById('chonde').innerHTML = '<?php echo "Bài ".$i; ?>'; de={i}; " data-de="<?php echo $i; ?>" class="getdata" href="/practice/class-5/subject-{subjectEntity.get('alias')}-{subject}/topic-{section2[alias]}-{section2[id]}/examination-{i}"><?php echo "Bài ".$i;?><?php if($check == 0){ echo " - Bài dùng thử"; }?></a></li>
													<?php 
													}
												} else {
													foreach ($topicChilds as $topic) {
														...
														$practices = $data->getPractices($class,$topic['id'], $check);
														for($i = 1; $i <= $practices; $i++){  ?>
															<li ><a style="padding-left: 50px;" onclick="document.getElementById('chonde').innerHTML = '<?php echo "Bài ".$i; ?>'; de={i}; " data-de="<?php echo $i; ?>" class="getdata" href="/practice/class-5/subject-{subjectEntity.get('alias')}-{subject}/topic-{topic[alias]}-{topic[id]}/examination-{i}"><?php echo "Bài ".$i;?><?php if($check == 0){ echo " - Bài dùng thử"; }?></a></li>
														<?php 
														} 
													}	
												}
