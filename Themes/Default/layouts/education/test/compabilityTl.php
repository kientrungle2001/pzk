<?php
	...
	<p class="t-weight text-center btn-custom8 textcl"><?=$language['essay-test'];?></p>
    ...
    function finish_choicetl(){
		...
		<?php } else { ?>
		var during_timetl = parseInt(start_time)*60 - (parseInt(time_real[0])*60 + parseInt(time_real[1]));
		<?php } ?>
	...
	<?php } else { ?>
	CountDowntl.Start(<?=$data_criteria['time']*60*1000?>);
	<?php } ?>
