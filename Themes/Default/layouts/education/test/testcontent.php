...
if(!$doTestPostUrl) {
 ...
 ?>
<div class="container">
	<p class="t-weight text-center btn-custom8 mgright textcl">Làm {ifvar type}bài luyện tập{else}đề thi{/if} - Lớp <?php echo $class; ?></p>
</div>
<div class="guide">
	<!-- Video slide guide -->
</div>
		    	...
		    	<div class="col-xs-9 pd-0">
		    		<select id="test" name="test" class="form-control select_type title-blue" onchange = "get_time_test(this.options[this.selectedIndex].getAttribute('data_time'))" >
		    			<?php if(is_array($testDetail) && $isInList):?>
							<option value="<?=$testDetail['id']?>" select="selected" data_time="--:--"><?php if(pzk_user_special()): ?>#{testDetail[id]}<?php endif;?> - <?=$testDetail['name']?> </option>
						<?php else:?>
