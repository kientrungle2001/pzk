...
class PzkProfileController extends PzkFrontendController 
{
	...
	{
		if(1 && pzk_themes('cms')) {
			$this->set('masterPage', 'onepage');
			$this->set('masterPosition', 'left');
	...
	public function editpassword1Action()
	{
	...
	public function sendMail($email = "",$key = "",$newpassword = "")
	{
		...
		$mailtemplate = $this->parse('user/mailtemplate/editpassword');
	...
	public function confirmpassAction()
	{
		...
		if($user->get('id'))
		{	
			$editpass = $this->parse('user/profile/editpasswordsuccess');
