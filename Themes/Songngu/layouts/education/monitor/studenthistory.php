...
<div class='container'>
	...
	<div class="well">
		...
		<select class="form-control" id="resultAchievement" name="viewWeek" onchange="window.location = '/monitor/resultAchievementByWeek?userId=<?=$userId;?>&val='+this.value;">
			<?php 
				$week = date('W');
				$crurrentWeek = date('W')-1;
				$year = date('Y');
				...
				if(pzk_session('resYearAchievement')){
					$year = pzk_session('resYearAchievement');
				}	
				
				$weekYear = $filterWeek.'-'.$year;
				
				if($crurrentWeek < 52 && $year > 2016) { 
					$k = 52 - $crurrentWeek;
					for($week= 52-$k; $week <= 52; $week++){
						?>
						<option value="<?php echo $week.'-'.$year-1;?>">Tuần <?php $date = startEndDateOfWeek($week, $year-1, 1); echo $week; echo ' ('.$date['startdate'].' đến '.$date['enddate'].')'; ?></option>
						<?php
				...
				?>
				<option value="<?php echo $week.'-'.$year;?>">Tuần <?php $date = startEndDateOfWeek($week, $year, 1); echo $week; echo ' ('.$date['startdate'].' đến '.$date['enddate'].')'; ?></option>
				<?php } ?>
	...
	<table class="table table-hover table-bordered">
			...
			<tr>
				...
				<td><?php $date = startEndDateOfWeek($filterWeek, $year, true); echo 'Tuần '.$filterWeek.'('.$date['startdate'].' đến '.$date['enddate'].')';?> </td>
			</tr>
			<tr>
				<th>Xếp hạng toàn quốc</th>
				<th><?php echo $data->getRateAchievement($achievement['id'], $filterWeek, $year, 'tree'); ?></th>
	...
	<h2 class="text-center robotofont">Lịch sử học tập</h2>
  	...
  	<div class="well">
		...
		<div class="row">
		<div class="col-md-4 col-sm-6 col-xs-12">
		<select class="form-control input-sm " id="viewWeek" name="viewWeek" onchange="window.location = '/monitor/filterDataByWeek?userId=<?=$userId;?>&val='+this.value;">
			...
			?>
			<option value="<?php echo $k.'-'.$year; ?>">Tuần 
			<?php $date = startEndDateOfWeek($k, $year, 1); echo $tong - $j; echo ' ('.$date['startdate'].' đến '.$date['enddate'].')'; ?></option>
			<?php
				if($j == 52){
					break;
				}
  ...
  <div class="tab-content">
    <div role="tabpanel" class="tab-pane active" id="luyentap">
		...
		<div class="panel panel-success"> 
			<div class="panel-heading">
			<a data-toggle="collapse" href="#mucdost">
				<h3 class="panel-title">
			...
			<div id="mucdost" class="panel-collapse panel-body collapse">
				<ul class="list-group">
				<?php 
							...
							<td>
								<?php if($pages > 1) { ?>
									...
									<ul style="margin: 0px; vertical-align: middle;" class="pagination">
										...
										if($data->pageNum >= 1) { ?>
											<li>
												<a onclick="practice({userId}, 0, '{startDate}', '{endDate}')" aria-label="End">
													<span aria-hidden="true">Trang đầu</span>
												</a>
											<li>
												<a aria-label="Previous" onclick="practice({userId},'<?php echo $data->pageNum -1; ?>', '{startDate}', '{endDate}')">
										...
										for ($page = 0; $page < $pages; $page++) { ?>
											...
											?>
											<li class="{active}">
												<a  onclick="practice({userId}, {page}, '{startDate}', '{endDate}')">{? echo ($page + 1)?}</a>
											</li>
										<?php } ?>

										<?php if($data->pageNum < $pages-1) { ?>
										<li>
											<a onclick="practice({userId}, '<?php echo $data->pageNum + 1; ?>', '{startDate}', '{endDate}')" aria-label="Next">
												<span aria-hidden="true">&raquo;</span>
											</a>
											</li>
											<li>
											<a onclick="practice({userId}, '<?php echo $pages-1; ?>', '{startDate}', '{endDate}')" aria-label="end">
							...
							<td>
								<?php if($pages > 1) { ?>
									...
									<ul style="margin: 0px; vertical-align: middle;" class="pagination">
										...
										if($data->pageNum >= 1) { ?>
											<li>
												<a onclick="getPage({userId},1,0, '{startDate}', '{endDate}', 'resPractice')" aria-label="End">
													<span aria-hidden="true">Trang đầu</span>
												</a>
											<li>
												<a aria-label="Previous" onclick ="getPage({userId}, 1, '<?php echo $data->pageNum -1; ?>', '{startDate}', '{endDate}', 'resPractice')">
													<span aria-hidden="true">&laquo;</span>
												</a>
											</li>
										<?php } ?>
										...
										for ($page = 0; $page < $pages; $page++) { ?>
											...
											?>
											<li class="{active}">
												<a  onclick = "getPage({userId}, 1, {page}, '{startDate}', '{endDate}', 'resPractice')">{? echo ($page + 1)?}</a>
											</li>
										<?php } ?>

										<?php if($data->pageNum < $pages-1) { ?>
										<li>
											<a onclick = "getPage({userId}, 1, '<?php echo $data->pageNum + 1; ?>', '{startDate}', '{endDate}', 'resPractice')" aria-label="Next">
												<span aria-hidden="true">&raquo;</span>
											</a>
											</li>
											<li>
											<a onclick =" getPage({userId}, 1, '<?php echo $pages-1; ?>', '{startDate}', '{endDate}', 'resPractice')" aria-label="end">
												<span aria-hidden="true">Trang cuối</span>
											</a>
										</li>
										<?php } ?>
							...
							<td>
								<?php if($pages > 1) { ?>
									...
									<ul style="margin: 0px; vertical-align: middle;" class="pagination">
										...
										if($data->pageNum >= 1) { ?>
											<li>
												<a onclick="getPage({userId},0,0, '{startDate}', '{endDate}', 'resTest')" aria-label="End">
													<span aria-hidden="true">Trang đầu</span>
												</a>
											<li>
												<a aria-label="Previous" onclick ="getPage({userId}, 0, '<?php echo $data->pageNum -1; ?>', '{startDate}', '{endDate}', 'resTest')">
													<span aria-hidden="true">&laquo;</span>
												</a>
											</li>
										<?php } ?>
										...
										for ($page = 0; $page < $pages; $page++) { ?>
											...
											?>
											<li class="{active}">
												<a  onclick = "getPage({userId}, 0, {page}, '{startDate}', '{endDate}', 'resTest')">{? echo ($page + 1)?}</a>
											</li>
										<?php } ?>

										<?php if($data->pageNum < $pages-1) { ?>
										<li>
											<a onclick = "getPage({userId}, 0, '<?php echo $data->pageNum + 1; ?>', '{startDate}', '{endDate}', 'resTest')" aria-label="Next">
												<span aria-hidden="true">&raquo;</span>
											</a>
											</li>
											<li>
											<a onclick =" getPage({userId}, 0, '<?php echo $pages-1; ?>', '{startDate}', '{endDate}', 'resTest')" aria-label="end">
												<span aria-hidden="true">Trang cuối</span>
											</a>
										</li>
										<?php } ?>
