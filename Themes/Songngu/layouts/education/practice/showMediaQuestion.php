					...
					<div class="row">
						...
						<?php } else { 
							if($check == 0){ 
								...
								echo $mediaEntity->get('name')." - "; if($lang == 'en' || $lang == 'ev'){ echo $topic['name_en']; }else{ echo $topic['name_vn']; }
							} ?>
									...
									foreach ($sections as $section) {
										...
										if(count($topicChilds) == 0) {
											...
											$medias		=	$data->getMedias($section['id']);
													foreach($medias as $media) {  ?>
														<li 
															class="list-group-item exercise-of-topic-{section[id]}  
															<?php if($currentMedia == $media['id']) echo 'active'; ?>">
																<a style="
																	<?php if(!@$section['trial'] && pzk_request('topic')!=$section['id'] ):?>display: none;<?php endif;?>
																	padding-left: 40px;" onclick="return check_display(1);"class="getdata" href="/practice/class-{class}/subject-{subjectEntity.get('alias')}-{subject}/topic-{section[alias]}-{section[id]}/media-{media[id]}">{media[name]}</a></li>
											<?php	}
													...
													for($i = 1; $i <= $practices; $i++){  ?>
														<li class="list-group-item 
															...
															style="<?php if(!@$section2['trial'] && pzk_request('topic')!=$section2['id']):?>display: none;<?php endif;?>">
																<a style="padding-left: 50px;" onclick="document.getElementById('chonde').innerHTML = '<?php echo $language['lesson'].$i; ?>'; de={i}; return check_display({section2[trial]});" 
																data-de="<?php echo $i; ?>" 
																class="getdata" 
											...
											<?php	}
														for($i = 1; $i <= $practices; $i++){  ?>
															<li class="list-group-item 
																...
																<?php if(pzk_request('topic')==$topic['id'] && pzk_request('de') == $i) echo'active'; ?>" 
																style="<?php if(pzk_request('topic')!=$topic['id']):?>display: none;<?php endif;?>">
																	<a style="padding-left: 50px;" onclick="document.getElementById('chonde').innerHTML = '<?php echo $language['lesson'].$i; ?>'; de={i}; return check_display({topic[trial]});" 
																	data-de="<?php echo $i; ?>" 
																	class="getdata" 
																	href="/practice/class-{class}/subject-{subjectEntity.get('alias')}-{subject}/topic-{topic[alias]}-{topic[id]}/examination-{i}">
																	<?php echo $language['lesson'].$i;?></a></li>
