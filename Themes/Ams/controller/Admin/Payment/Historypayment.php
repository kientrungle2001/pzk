...
class PzkAdminPaymentHistorypaymentController extends PzkGridAdminController {
	public $addFields = ' username, paymentType,bank,transactioncode,billcode, amount, paymentStatus,paymentDate,expiredDate, status, software,buySoftware, serviceType,site, contestId, serviceId, categoryIds, class, categoryIds,languages,coupon,contestIds';
	public $editFields = ' username, paymentType,bank,transactioncode,billcode, amount, paymentStatus,paymentDate,expiredDate, status,software,buySoftware, serviceType,site, contestId, serviceId, categoryIds, class , categoryIds,languages,coupon,contestIds';
	...
	public $addFieldSettings = array(
		...
		array(
			...
			'show_name' 	=> 'name',
			'mdsize'		=> 12,
			'condition'		=> 'compability=1 and parent=0 and status=1'
		),
		array(
			'index' 		=> 'paymentType',
			'type' 			=> 'selectoption',
    ...
    public function addPostAction() {
        ...
        if(isset($row['serviceId'])) {
			...
			if($service) {
				...
				$row['contestIds']	= @$row['contestIds'] ? $row['contestIds']: $service['contestIds'];
