				...
				<div class="item border-question" style="z-index: 9">
					<form id="form_question_nn" class="question_content pd-0 item mgb15 form-horizontal bgclor" method="post">
						<div class="item margin-top-20 scrollquestion">
							...
							<input type="hidden" id="parentTestId" name="parentTestId" value="<?=$parentTestId;?>" />
						</div>
	...
	<div class="modal-dialog">
      	<div class="modal-content">
	          	...
	          	<div class="container-fluid">
	           		<div class="row">
						<div class="col-xs-12">
							...
							<h3 class="text-center text-danger">Kết quả sẽ được công bố vào ngày <?php echo date('d/m/Y', strtotime($parentTest['resultDate']));?></h3>
						</div>
		    		</div>
	          	</div>
	        </div>
	        <div class="modal-footer">
				...
				<button class="btn btn-primary" name="continue" onclick="$('#completeModal').modal('hide'); window.location.reload();" type="button"><span class="glyphicon glyphicon-check"></span> 
				<?php if($ordering == 1):?>
				Làm đề tiếp theo
				<?php else: ?>
				Hoàn thành
	...
	function save_choice(callback){
		if(formdata	==	null){
			...
			$.ajax({
				...
				url:'<?=BASE_REQUEST?>/Compability/saveContestMixedTest',
				success: function(results){
					if(callback) {
						callback();
					}
