	...
	<div class="row">
		<div class="content-full col-xs-12 ">
						...
						<div class="item margin-top-20 scrollquestion">
							<?php if($book['status']) : ?>
								<table  class="table table-bordered">
									<tr>
										<th class="col-xs-3 text-center">Điểm</th>
			...
			<div class="col-md-12 col-sm-12 fix_da">
				<button id="finish-choice" class="btn btn-primary btt-practice" disabled>
					<span class="glyphicon glyphicon-ok"></span>
						<span class="btn-finish">Đã nộp</span>
				</button>
...
<script>
	...
	function show_result_only() {
		$.ajax({
			...
			url:'<?=BASE_REQUEST?>/Compability/showMixedTestResultDialog',
	...
	function show_answers(){
       	...
       	if(formdata	==	null){
      		...
      		$.ajax({
		        ...
		        url:'<?=BASE_REQUEST?>/Compability/showMixedTestAnswersChoice',
		        success: function(results){
		         	var data = $.parseJSON(results);
		         	var user_answer = '';
			...
			if(user_book_id == undefined){
	        	if(formdata == null){
	          	...
	          	$.ajax({
		            ...
		            url:'<?=BASE_REQUEST?>/Practice/saveHomework',
		            success: function(results){
		            	if(callback) {
							callback();
						}
