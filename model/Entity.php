...
class PzkEntityModel extends PzkSG{
	...
	public function update($data) {
		...
		if(0) {
			foreach($data as $key => $value) {
				if(!in_array($key, $fields)) {
					$option = _db()->select('*')->from('table_option')
						->whereItemId($this->get('id'))
						->whereTable($this->table)
						->whereName($key)
						->result_one();
					if(!$option) {
						_db()->insert('table_option')->fields('`table`,`name`,`value`,`itemId`')
						->values(
						array(
							array(
								'name'	=> $key,
		...
		_db()->update($this->table)->set($data)->whereId($this->get('id'))->result();
	}
	public function save() {
		...
		if(!isset($this->data['id']) || !$this->data['id'] ) {
			...
			if($this->isLock()) {
				...
				_db()->update($this->table)->set($data)->where(array('id', $this->get('id')))->result();
				return true;
			}
		}
		$fields = _db()->getFields($this->table);
	...
	public function updateParents() {
		if(isset($this->data['parent'])) {
			...
			if($parent) {
				...
				$obj->load($parent);
				$parents = $obj->get('parents') . $this->get('id') . ',';
			} else {
				$parents = ','.$this->get('id') . ',';
			}
	...
	public function children($action = false) {
		...
		$children = $this->getWhere(array('parent', $this->get('id')));
