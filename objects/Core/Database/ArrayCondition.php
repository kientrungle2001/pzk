...
function mf_build($cond) {
	// nếu cond là mảng
	if(is_array($cond)) {
		// nếu mảng này dạng key => value thì xét nó như dạng and
		if(!isset($cond[0])) {
			$rs = array('and');
			foreach($cond as $key => $val) {
				$rs[] = array($key, $val);
		...
		if(in_array($op, PzkCoreDatabaseArrayCondition::$operations)) {
			...
			if(count($cond) >= 2 ) {
				if(!is_array($cond[0]) && !is_array($cond[1])) {
					...
					return call_user_func_array('mf_and', $cond);
				}
			}
			return $op;
		}
	} else {
		// ngược lại, là chuỗi hoặc số
		return $cond;
	}
}
...
function mf_equal($exp1, $exp2) {
	...
	return '(' . mf_build($exp1) .'=' . mf_build($exp2) . ')';
}
...
function mf_like($exp1, $exp2) {
	...
	return '(' . mf_build($exp1) .' like ' . mf_build($exp2) . ')';
}
...
function mf_notlike($exp1, $exp2) {
	...
	return '(' . mf_build($exp1) .' not like ' . mf_build($exp2) . ')';
}
