<?php 
class PzkEducationUserbookTypeTrytesttl extends PzkObject{
	
	public $layout = 'userbook/trytesttl';
	...
	function getQuestion(){
		...
		if(isset($this->questionId)){
			...
			$query = _db()->useCache(1800)->select('id, request, teacher_answers, name, type, topic_id, type_id')
			->from('questions')
	...
	function getAnswersAccepts(){
	
		if(!empty($this->id)){
				
			$query = _db()->select('accept') ->from('user_answers') ->where(array('id', $this->id));
	...
	public function getStudentAnswers() {
		if(!empty($this->id)){
			...
			$query = _db()->select('*') ->from('user_answers') ->where(array('id', $this->id));
