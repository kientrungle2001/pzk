...
<span class="inline-edit" id="inline-item-<?php  echo $data->getIndex() ?>-<?php  echo $data->getItemId() ?>">
	<span class="inline-text" ondblclick="pzk_list.showInlineEdit('<?php  echo $data->getIndex() ?>', <?php  echo $data->getItemId() ?>); return false;">
		...
		if(@$data->getLink()): ?>
		<a target="<?php echo $data->getTarget()?>" href="<?php eval('?>'.PzkParser::parseTemplate($data->getLink(), $data) . '<?php '); ?><?php  echo $data->getItemId() ?>"><?php echo $value ?></a>
		<?php else:?>
		<?php echo $value ?>
		<?php endif;?>
		<?php if(0):?>
	...
	<span class="inline-input" style="display: none;">
		<input class="inline-input-field" type="text" name="<?php  echo $data->getIndex() ?>" value="<?php  echo html_escape($data->getValue()) ?>" 
