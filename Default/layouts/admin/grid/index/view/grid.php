		...
		<div class="panel panel-default">
			...
			<table id="admin_table_<?php  echo $data->getId() ?>" class="table table-hover table-bordered table-striped table-condensed">
				...
				<tr id="row-<?php echo @$item['id']?>" class="row-item row-item-<?php echo @$item['id']?> row-parent-<?php echo @$item['parent']?>" rel="<?php echo @$item['id']?>">
					...
					?>
				<td <?php if($isOrderingField): ?>style="white-space: nowrap;"<?php endif; ?>><span class="column-<?php echo @$field['index']?>"><?php  $fieldObj->display(); ?></span><?php  if($fieldObj->getLink()): ?> <a href="<?php  echo $fieldObj->getLink()?><?php  echo $fieldObj->getItemId() ?>"><span class="glyphicon glyphicon-link"></span></a><?php  endif;?></td>
					<?php endforeach; ?>
					<?php if(${'normalMode'}): ?>
					<td style="white-space: nowrap"><?php if($data->getCheckEdit()) { ?><a href="<?php echo BASE_REQUEST . '/Admin' ?>_<?php  echo $data->getModule() ?>/edit/<?php echo @$item['id']?>" class="text-center"><span class="glyphicon glyphicon-edit"></span></a><?php } ?>
